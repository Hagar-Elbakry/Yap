            ...
            @forelse($notifications as $notification)
                 <div class="flex p-4 border-b {{$loop->last ? '' : 'border-b-gray-400'}}">
                    ...
                    @elseif($notification->type == 'App\Notifications\PostReacted' && $notification->notifiable_id == currentUser()->id)
                         <p><span class="font-bold">{{$notification->data['name']}}</span> {{$notification->data['reaction']}} your post <a href=""><span class="font-bold text-gray-500">{{$notification->data['post']}}</span></a></p>
                    @endif
                 </div>
            @empty
